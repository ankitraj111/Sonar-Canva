...
// Radar configuration
const minR = 0, maxRVal = 800;
const fov = 100; // ±50 degrees
const numCircles = 4;
...
// Targets array with lightlong trails
const targets = [
...
// Helper to normalize angle between -180° and 180°
...
// Update target trail points (lightlong effect)
function updateTrail(target) {
  target.trail.push({ range: target.range, angle: target.angle });
  if (target.trail.length > 20) target.trail.shift();
}

// Move targets and update trails
...
// Draw targets with glowing lightlong trails and labels
function drawTargets() {
  ...
  targets.forEach(t => {
    // Glowing trails (lightlong effect)
...
// Draw scanning beam (commented by default)
function drawBeam() {
...
// Sonar info panel for visible targets inside FOV
...
function animate() {
  ...
  // Uncomment below lines to enable beam rotation and drawing
