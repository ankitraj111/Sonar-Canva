 const canvas = document.getElementById('sonar');
  ...
  // scanning beam is disabled for now. Uncomment to enable.
  /*
  function drawBeam() {
  ...
  */
  ...
  // Optional info panel on canvas
  function drawInfoPanel() {
    const boxW = 180;
    const boxH = 20 + targets.length * 20;
    const padding = 8;
    const x = canvas.width - boxW - 20;
    const y = 20;

    ctx.fillStyle = "rgba(24, 39, 66, 0.85)";
    ctx.fillRect(x, y, boxW, boxH);

    ctx.fillStyle = "#ffee88";
    ctx.font = "16px Arial";
    ctx.fillText("SONAR INFO", x + padding, y + 24);
    ...
    targets.forEach((t, i) => {
      ...
      ctx.fillText(text, x + padding, y + 50 + i * 20);
    });
  }
  ...
  function animate() {
    ...
    // Uncomment to enable scanning beam
    // drawBeam();
